...
export const aya2id = ({ sura, aya }, sura_Aya) => {
  if (sura_Aya) {
    let suraAya = sura_Aya;
    sura = suraAya.split("_")[0];
    aya = suraAya.split("_")[1];
  }
  let id = QuranData.Sura[sura][0];
  id += Number(aya);

  return id;
};

export const id2aya = (id, numeric) => {
  // @IMPROVE
  let i = 1;
  let temp_id = id;
  while (temp_id > 0) {
  ...
  let sura = i - 1;
  let aya = QuranData.Sura[sura][1] + temp_id;
  return numeric ? paddingAya(sura) + "" + paddingAya(aya) : { sura, aya }; //sura + "_" + aya;
};
...
export const getAyatBySuraAya = d =>
  ayatJson
    .filter(itm =>
      parseInt(d.sura) == itm[1] && parseInt(d.aya) == itm[2] ? true : false
    )
    .map(el => ({
      id: el[0],
      sura: el[1],
...
export const pageToSuraAya = page => {
  const sa = QuranData[page_key][page];
...
export const calcKhitma = ({ juz, day, res, lang, _lang }) => {
  ...
  txtWerd +=
    ...
    aya +
    ", ";
  const starPage = getAyatBySuraAya({ sura, aya }).page;
  ...
  txtWerd +=
    ...
    aya +
    ", ";
  const endPage = getAyatBySuraAya({ sura, aya }).page;
...
export const fetchJSON = url => fetch(url).then(response => response.json()).catch(err=>alert(err));

export const fetchText = url => fetch(url).then(response => response.text()).catch(err=>alert(err));
export const wait = (ms = 200) => new Promise((resolve) => setTimeout(resolve, ms));
