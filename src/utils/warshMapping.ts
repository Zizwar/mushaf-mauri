...
type OffsetException = {
  type: "offset";
  mergeAtWarshAyah: number;
  hafsTargetsToMerge: number[];
  offsetForSubsequent: number;
};

type AdvancedException = {
  type: "advanced";
  explicit_map: Record<number, number[]>;
  ranges: { warshStart: number; warshEnd: number; hafsOffset: number }[];
};

const WARSH_EXCEPTIONS: Record<number, ManualException | OffsetException | AdvancedException> = {
  // ──────────────────────────────────────────────────────────────────────────
  ...
  // W1 merges المص+كتاب أنزل (H1+H2). H137 split into W136+W137. Offset=0 from W138.
  7: {
    type: "advanced",
  ...
  // W1 merges طسم+تلك آيات (H1+H2). H23 split into W22+W23. Offset=0 from W24.
  28: {
    type: "advanced",
  ...
  // W1 merges الم+أحسب الناس (H1+H2). H29 split into W28+W29. Offset=0 from W30.
  29: {
    type: "advanced",
  ...
  // W1 merges الم+تنزيل الكتاب (H1+H2). H10 split into W9+W10. Offset=0 from W11.
  32: {
    type: "advanced",
  ...
  // W1 merges حم+والكتاب المبين (H1+H2). H52 split into W51+W52. Offset=0 from W53.
  43: {
    type: "advanced",
  ...
  // Al-'Alaq (96): W=20, H=19 (diff=-1). Warsh splits Hafs A15 into W15+W16. W17+ → offset -1.
  96: {
    type: "advanced",
  ...
  // Az-Zalzalah (99): W=9, H=8 (diff=-1). Warsh splits Hafs A6 into W6+W7. W8-9 → offset -1.
  99: {
    type: "advanced",
  ...
  // Quraysh (106): W=5, H=4 (diff=-1). Warsh splits Hafs A4 into W4+W5.
  106: {
    type: "advanced",
    explicit_map: { 4: [4], 5: [4] },
    ranges: [
  ...
  // Al-Ma'un (107): W=6, H=7 (diff=+1). Warsh merges Hafs A6+A7 into W6.
  107: {
    type: "advanced",
    explicit_map: { 6: [6, 7] },
    ranges: [
  ...
  // H8→W8+W9, H9→W10+W11; H18→W20+W21; W25=[H22,H23]; H41→W43+W44.
  56: {
    type: "advanced",
  ...
  // H24→W24+W25, H25→W26+W27; W28+ at off-2.
  71: {
    type: "advanced",
 ...
 * Returns an array of Hafs ayah numbers (usually [n], sometimes [n, n+1] for merges).
 *
 * @param sura - Surah number (1–114)
 * @param warshAya - Ayah number in Warsh (Madani) numbering
 * @returns Array of Hafs (Kufi) ayah numbers
 */
