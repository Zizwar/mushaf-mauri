...
const k = withRDLength(array(y));

const C = withRDLength(b);

const v = withRDLength(array(p));

const O = withRDLength(array(w));

const L = withRDLength(u);
...
const D = {
  bytes(e) {
    ...
    if (e.ipv6hint) {
      t += O.bytes(e.ipv6hint) + 2;
    }
    if (e.echconfig) {
      t += L.bytes(e.echconfig) + 2;
    }
    if (e.dohpath) {
      t += L.bytes(e.dohpath) + 2;
    }
    if (e.odoh) {
      t += L.bytes(e.odoh) + 2;
    }
  ...
  write(e, t, s) {
    ...
    if (s.ipv4hint) {
      e.setUint16(t, 4);
      t = v.write(e, t + 2, s.ipv4hint);
    }
    if (s.ipv6hint) {
      e.setUint16(t, 6);
      t = O.write(e, t + 2, s.ipv6hint);
    }
    if (s.echconfig) {
      e.setUint16(t, 5);
      t = L.write(e, t + 2, s.echconfig);
    }
    if (s.dohpath) {
      e.setUint16(t, 7);
      t = L.write(e, t + 2, s.dohpath);
    }
    if (s.odoh) {
      e.setUint16(t, 32769);
      t = L.write(e, t + 2, s.odoh);
    }
    return t;
  },
  read(e, t) {
    ...
    while (t.offset - r < s) {
      ...
      switch (s) {
       ...
       case 4:
        n.ipv4hint = v.read(e, t);
        break;

       case 6:
        n.ipv6hint = O.read(e, t);
        break;

       case 5:
        n.echconfig = L.read(e, t);
        break;

       case 7:
        n.dohpath = f.decode(L.read(e, t));
        break;
...
const interfaceBindings = (e, t) => {
  ...
  if (s && t === oe.v6) {
    ...
    s.sort((e, t) => e.address < t.address ? -1 : 1);
  }
...
const interfaceBindingKeys = e => {
  const t = new Set;
  for (const s of e) {
    if (s.family === oe.v6) {
      t.add(`${s.address}/${s.netmask}#${s.scopeid ?? ""}`);
    } else {
      t.add(`${s.address}/${s.netmask}`);
    }
  }
  return t;
};

const compareInterfaceBindingKeys = (e, t) => {
  if (e.size !== t.size) {
    return !1;
  }
  for (const s of e) {
...
const createInterfaceSocket = (e, r, n) => {
  ...
  function setupSocket(e, t) {
    const s = t !== c?.settings ? c?.settings : null;
  ...
  function sendImmediate(e) {
    try {
      if (c) {
        c.socket.send(e.message, 0, e.message.byteLength, e.port, e.address, e.onSent);
      } else {
        e.onSent(null);
      }
    } catch (t) {
  ...
  const f = {
    ...
    get bindings() {
      return c?.settings.bindings ?? [];
    },
    async send(e) {
      if (c) {
        await send(e, c.settings.multicastAddress, le);
      }
    },
    refresh() {
      if (c) {
...
function createInterfaceAdvertiser(e, t, s) {
    ...
    while (3 === u) {
      await c.schedule(3, async r => {
        ...
        if (!y.refresh() || y.closed) {
          if (++t >= (y.setup ? 5 : 2)) {
            u = 4;
            s.onError(new Error(y.setup ? `mDNS unavailable on interface "${e}" after ${t} setup attempts` : `mDNS interface "${e}" failed initial setup after ${t} attempts`));
            return;
          }
          return r.retry();
        }
  ...
  return {
        ...
        await async function run() {
          while (1) {
            ...
            if (3 === u) {
              if (++l >= 15) {
                u = 4;
                s.onError(new Error(`mDNS unable to maintain stable advertising on interface "${e}" after ${l} state cycles`));
              }
            }
          }
        }();
    ...
    get bindings() {
      if (!p && !y.closed) {
        return y.bindings;
      } else if ("IPv4" === t.stack) {
        return interfaceBindings(e, oe.v4) ?? [];
      } else if ("IPv6" === t.stack) {
        return interfaceBindings(e, oe.v6) ?? [];
      } else {
        return [ ...interfaceBindings(e, oe.v4) ?? [], ...interfaceBindings(e, oe.v6) ?? [] ];
      }
    },
    async close() {
      try {
...
exports.advertise = function advertise(e) {
  ...
  return function advertiseInternal(e, t) {
    ...
    const r = new Map;
    const n = new Map;
    const a = t.createScheduler();
    const createHandle = s => {
      const a = createInterfaceAdvertiser(s, e, t);
      a.promise.then(() => {
        if (r.get(s) === a) {
          n.set(s, interfaceBindingKeys(a.bindings));
        }
      }).catch(e => {
        if (!AbortError.isAbortError(e)) {
          t.onError(e);
    ...
    for (const e of s) {
      r.set(e, createHandle(e));
    }
    a.schedule(3, e => {
      try {
        const e = new Set(t.networkInterfaceNames());
        for (const [t, s] of r) {
          if (!e.has(t)) {
            r.delete(t);
            n.delete(t);
        ...
        for (const [e, t] of r) {
          if (t.settled) {
            const s = interfaceBindingKeys(t.bindings);
            const a = n.get(e);
            if (void 0 === a) {
              n.set(e, s);
            } else if (!compareInterfaceBindingKeys(a, s)) {
              r.delete(e);
              n.delete(e);
            }
          }
