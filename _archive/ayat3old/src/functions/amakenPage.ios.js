import { WARSH } from "../amaken";
import { hlMeta } from "./hlmeta";
import { Dimensions } from "react-native";
import {aya2id} from '../functions'
const scrW_mosshaf = Dimensions.get("window").width+27;
const SPECIFY_LEFT = -10
const SPECIFY_TOP = 10


export const amakenPage = page => {
  const currMosshaf = "warsh";
  let amaken = WARSH[page]; //[page];
  //alert(JSON.stringify(WARSH[page]));
  //wino.tafssir[page]
...
function hl_draw(id, top, left, width, height, b_top = 0, b_left = 0, wino) {
  ...
  left = ((scrW_mosshaf / 456) * left) + SPECIFY_LEFT
  top = ((scrW_mosshaf / 456) * top) + SPECIFY_TOP;
