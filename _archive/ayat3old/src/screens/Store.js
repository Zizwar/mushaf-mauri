...
import { listAuthorTafsir, listAuthorTarajem,listPage } from "../amaken";
...
import { id2aya } from "../functions";

const DIR = FileSystem.documentDirectory;
const folderSqlit = `${DIR}SQLite`;
const folderHafs = `${DIR}hafs`;
const folderWarsh = `${DIR}warsh`;
const folderTajweed = `${DIR}tajweed`;
...
const FADED_THEME_COLOR = "#007affc4";
...
async checkFolderTarajem(){
 ...
 async deleteflders(folder,quira) {
  ...
  const {setDownloads} = this.props;
	let folderInfo = await FileSystem.deleteAsync(folder);
	 folderInfo = await FileSystem.getInfoAsync(folder);
	 if(quira)
	 setDownloads(0,quira)
	this.checkFolderTarajem();
  ...
  //
  setIndic = (author, statement,loading) => {
    this.setState(state => {
...
return false;
  ...
  downloadImagesPage = async (quira) => {
   ...
   const {setDownloads} = this.props;
     const exist = await this.isExistFloder(folderQuira)
     
    
 if(exist && (NUM_PAGE===page)) return alert("exist page");
   ...
   // await FileSystem.makeDirectoryAsync(folderQuira);
    ...
    const itemChunk = listNumImg.map(async id => {
      const uri = getImagePageUri({ id, quira });
      console.log({ uri });
      const downloadedHafs = await FileSystem.downloadAsync(
        uri,
        folderQuira + "/" + id + ".png"
      );
      console.log({ downloadedHafs });
	  page++;
  ...
  downloadVoice = async (moqri = "Hudhaify_64kbps") => {
...
if(tarajemDB.includes(db + ".db"))
	return true;
return false;

}
isExistPage = (quira)=>{
	const { downloadsHafs,downloadsHafsMadina,downloadsWarsh,downloadsTajweed} = this.props;
console.log("QUIRA ",{ downloadsHafs,downloadsHafsMadina,downloadsWarsh,downloadsTajweed,});

switch (quira) {
    case "hafs": return downloadsHafs;
  case "hafsMadina":  return downloadsHafsMadina;
  case "warsh":  return downloadsWarsh;
  case "tajweed": return downloadsTajweed
	   default: return 0;
  };
}
...
*/
  ...
  renderItemTafsir = (data, i) => (
    <ListItem
      onPress={() => {
        ...
        this.openModalStorTafisrTarajem(this.lang["bu_download_ttarajem"], data);
 ...
 this.confirm(()=> this.deleteflders(`${folderSqlit}/${data.id}.db`),data.name);
		 // this.setIndic(data.id,true);
      }}	  	  
	  >
          {this.isExistTarajem(data.id)?<Icon name="close" />:((this.state.tafsirOrTarajem == this.lang["download_ttarajem_tarajem"]) && this.state.thisIndic[data.id] ? (
      ...
      <Body>
        <Text style={{color:this.props.theme.color}}>{data.name}</Text>
      </Body>
      <Right />
    </ListItem>
  );
  ...
  renderItemTarajem = (data, i) => (
    <ListItem
      onPress={() => {
        //this.goBack();
        //this.props.setExactAya(data.id);
        this.openModalStorTafisrTarajem(this.lang["download_ttarajem_tarajem"], data);
      }}
      avatar
    >
      <Button transparent
  ...
  renderItemPage = (data, i) => (
    ...
    >
      ...
      onPress={() => {
		 if(this.isExistPage(data.id))
	   this.confirm(()=> this.deleteflders((DIR+data.id),data.id),data.name);
               else
               this.downloadImagesPage(data.id);
	
      }}	  
	  >
        { this.isExistPage(data.id)===NUM_PAGE?<Icon name="close" />:(this.state.thisIndic[data.id] ? (
          <ActivityIndicator animating={true} size={32} />
        ) : this.isExistPage(data.id)? <Icon name="pause" />:(
          <Icon name="download" />
        ))}
      </Button>
  ...
  //nScroll = new Value(0);
 // scroll = new Value(0);
  textColor = this.props.theme.color;/*this.scroll.interpolate({
    inputRange: [0, SCROLL_HEIGHT / 5, SCROLL_HEIGHT],
    outputRange: [this.THEME_COLOR, this.FADED_THEME_COLOR, "white"],
    extrapolate: "clamp"
  });*/
  tabBg =  this.props.theme.backgroundColor;/* this.scroll.interpolate({
  ...
  render() {
    const {
	  ...
	  const {theme: { color: backgroundColor }}=this.props;
      ...
      <View>
        ...
        >
          <Header>
            ...
            <Body>
              <Title>{this.lang["bu_download_cnt"]}</Title>
            </Body>
            <Right />
          </Header>
          {/*
        ...
        >
          ...
          >
            <Tab heading={this.lang["download_ttarajem_tarajem"]}>
              <List
                dataArray={
                  this.state.listViewDataTarajem}
                // dataArray={datas}
                renderRow={this.renderItemTarajem}
                renderLeftHiddenRow={data => <Button full></Button>}
                leftOpenValue={75}
                rightOpenValue={-75}
              />
            </Tab>
            <Tab heading={this.lang["bu_download_ttarajem"]}>
              <List
                dataArray={
            ...
            <Tab heading={this.lang["pages" ]}>
              <List
                dataArray={
                  this.state.listViewDataPage}
                // dataArray={datas}
                renderRow={this.renderItemPage}
...
const mapStateToProps = ({
	wino,
		lang,
		author,
		downloadsHafs,downloadsHafsMadina,downloadsWarsh,downloadsTajweed,
		options, quira,imagesDB,tarajemDB,theme }) => ({
  wino,
  downloadsHafs,downloadsHafsMadina,downloadsWarsh,downloadsTajweed,
  author,
  lang,
